...
function CarCard({ car }: { car: Car }) {
  return (
    <Link href={`/collection/${car.id}`} className="group relative bg-stone-900 rounded-2xl overflow-hidden card-3d">
      <div className="img-zoom relative aspect-[16/10] overflow-hidden">
        ...
        {car.angles && (
          <div className="absolute top-4 right-4">
            <span className="px-2.5 py-1 glass text-xs text-green-400 rounded-full">360°</span>
          </div>
        )}
      </div>
      <div className="p-6">
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <ScrollReveal>
          <div className="text-center mb-16">
            ...
            <p className="text-stone-400 max-w-xl mx-auto">
              A curated selection of extraordinary automobiles, each chosen for its unique character and driving experience.
            </p>
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal delay={200}>
          <div className="flex flex-wrap justify-center gap-3 mb-12">
            {categories.map((cat) => (
              <span key={cat} className="px-5 py-2 text-sm tracking-wide glass text-stone-300 rounded-full cursor-default hover:bg-white/10 transition-colors">{cat}</span>
            ))}
          </div>
        </ScrollReveal>
        ...
        <div className="text-center mt-16">
          <p className="text-stone-500 text-sm">Don't see what you're looking for? <Link href="/contact" className="text-white link-underline">Tell us what you want</Link></p>
        </div>
      </div>
    </main>
  );
